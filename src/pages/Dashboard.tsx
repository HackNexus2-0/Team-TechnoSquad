...
import { portfoliosService, transactionsService } from '../lib/firestore';
...
export default function Dashboard() {
  ...
  const loadPortfolios = async () => {
    try {
      if (!user) return;
      const data = await portfoliosService.getAll(user.uid);
      setPortfolios(data || []);
  ...
  const loadPortfolioMetrics = async (portfolioId: string) => {
    try {
      const transactions = await transactionsService.getByPortfolio(portfolioId);

      const holdings: Map<string, Holding> = new Map();
      let totalCost = 0;

      transactions?.forEach((txn: any) => {
        const stock = txn.stock;
        if (!stock) return;
